...
void DrawShadedCubeV(Vector3 center, Vector3 size, Color base) {
    ...
    // Solid five-face lighting. Objects stand on the terrain board, so the bottom
    // face must not be rendered: if it appears from an isometric view, the block
    // reads like a hollow floating box. Keep only the top and four side faces.
    ...
    DrawQuad3D(v010, v110, v111, v011, top);       // top
    DrawQuad3D(v100, v101, v111, v110, east);      // +X side
    DrawQuad3D(v000, v010, v011, v001, west);      // -X side
    DrawQuad3D(v001, v011, v111, v101, south);     // +Z side
    DrawQuad3D(v000, v100, v110, v010, north);     // -Z side
}

void DrawFlatShadedCubeV(Vector3 center, Vector3 size, Color base) {
    // Thin slabs such as road/water should read as clean solid plates, not dark plates
    // with black rims. Use only a very gentle side shade.
    const Color top = ScaleColor(base, 1.06f);
    const Color side = ScaleColor(base, 0.92f);
    DrawShadedCubeV(center, size, side);
    DrawCubeV({ center.x, center.y + size.y * 0.505f, center.z }, { size.x, 0.006f, size.z }, top);
}
...
Color ProfileColor(MovementProfile profile, ThemeMode theme) {
    const bool dark = theme == ThemeMode::Dark;
    switch (profile) {
        case MovementProfile::Pedestrian: return dark ? Color{ 255, 214, 139, 245 } : Color{ 231, 111, 81, 245 };
        case MovementProfile::Car:        return dark ? Color{ 119, 216, 255, 245 } : Color{ 50, 145, 205, 245 };
        case MovementProfile::Offroad:    return dark ? Color{ 132, 225, 166, 245 } : Color{ 59, 158, 112, 245 };
        case MovementProfile::Drone:      return dark ? Color{ 220, 166, 255, 245 } : Color{ 138, 91, 198, 245 };
        default: return dark ? Color{ 255, 245, 214, 245 } : Color{ 93, 64, 94, 245 };
    }
}

std::string ProfileDisplayName(MovementProfile profile) {
    switch (profile) {
...
void App::Run() {
    ...
    InitWindow(kWindowWidth, kWindowHeight, "PathLab Prototype v1.1 - demo, presets, undo, legend and exports");
...
void App::Update() {
    ...
    if (std::abs(wheel) > 0.001f && CheckCollisionPointRec(mouse, leftPanel_)) {
        leftPanelScroll_ = std::clamp(leftPanelScroll_ + wheel * S(54.0f), -S(760.0f), 0.0f);
    } else if (std::abs(wheel) > 0.001f && CheckCollisionPointRec(mouse, rightPanel_)) {
        rightPanelScroll_ = std::clamp(rightPanelScroll_ + wheel * S(54.0f), -S(660.0f), 0.0f);
    } else if (view3D_) {
        UpdateCamera3D();
...
void App::ResetCamera3D() {
    ...
    // Editor view is for precise picking and editing. Diorama view is for presentation,
    // so it uses a lower, closer camera and a slightly narrower FOV for a sandbox feel.
    cameraTarget_ = { 0.0f, smoothTerrain3D_ ? 0.55f : 0.0f, 0.0f };
    cameraDistance_ = std::max(smoothTerrain3D_ ? 24.0f : 28.0f, mapSize * (smoothTerrain3D_ ? 0.82f : 0.95f));
    cameraYaw_ = 0.78f;
    cameraPitch_ = smoothTerrain3D_ ? 0.70f : 0.58f;
...
void App::DrawTopBar() {
    ...
    DrawTextUI("v1.1", S(128), S(24), 15, MutedTextColor());
...
void App::DrawLeftPanel() {
    ...
    if (Button({ x + bw + gap, y, bw, S(34) }, "Clear")) {
        PushUndoState("Clear map");
        map_.ClearAll();
        ResetVisualization();
        statusMessage_ = "Map cleared";
    }
    y += S(40);
    if (Button({ x, y, bw, S(34) }, "City preset")) GenerateCityPreset();
    if (Button({ x + bw + gap, y, bw, S(34) }, "River preset")) GenerateRiverPreset();
    y += S(40);
    if (Button({ x, y, bw, S(34) }, "Mountain pass")) GenerateMountainPreset();
    if (Button({ x + bw + gap, y, bw, S(34) }, "Demo A*")) RunDemoMode();
    y += S(40);
    ...
    y += S(40);
    if (ToggleButton({ x, y, contentW, S(34) }, showAgentComparison_ ? "Agent paths: ON" : "Agent paths: OFF", showAgentComparison_)) {
        if (!showAgentComparison_) CompareAgents();
        else { showAgentComparison_ = false; agentComparisonRows_.clear(); statusMessage_ = "Agent comparison hidden"; }
    }
    y += S(46);
    ...
    y += S(40);
    if (ToggleButton({ x, y, bw, S(34) }, smoothTerrain3D_ ? "Style: diorama" : "Style: editor", smoothTerrain3D_)) {
        smoothTerrain3D_ = !smoothTerrain3D_;
        if (view3D_) ResetCamera3D();
        statusMessage_ = smoothTerrain3D_ ? "Diorama mode: presentation sandbox view" : "Editor mode: precise solid-grid editing";
    }
...
void App::DrawRightPanel() {
    ...
    y += 4 * (gh + gap) + S(12);
    if (Button({ x, y, halfW, gh }, "Undo")) UndoEdit();
    if (Button({ x + halfW + gap, y, halfW, gh }, "Redo")) RedoEdit();
    y += S(42);
    ...
    y += S(40);
    if (Button({ x, y, halfW, gh }, "Screenshot")) ExportScreenshot();
    if (Button({ x + halfW + gap, y, halfW, gh }, "CSV")) ExportBenchmarkCsv();
    y += S(38);
    DrawTextUI("Map file: " + CompactDisplayPath(currentMapPath_), x, y, 14, MutedTextColor());
    y += S(34);

    section("LEGEND");
    DrawLegend(x, y, contentW);
    y += S(10);
    ...
    if (showAgentComparison_ && !agentComparisonRows_.empty()) {
        DrawTextUI("AGENT PATHS", x, y, 16, TextColor());
        y += S(24);
        for (const auto& row : agentComparisonRows_) {
            Color c = ProfileColor(row.profile, themeMode_);
            DrawRectangleRec({ x, y + S(4), S(11), S(11) }, c);
            DrawTextUI(row.profileName + (row.found ? (" | " + FormatDouble(row.pathLength, 1)) : " | no path"), x + S(18), y, 13, row.found ? TextColor() : DangerColor());
            y += S(20);
        }
        y += S(8);
    }
...
void App::DrawLegend(float x, float& y, float contentW) {
    ...
    const float sw = S(13.0f);
    for (const auto& item : items) {
        Color color = TerrainVisualColor(item.terrain, 0.45f, false);
        DrawRectangleRounded({ x, y + S(3), sw, sw }, 0.18f, 6, color);
        DrawRectangleRoundedLines({ x, y + S(3), sw, sw }, 0.18f, 6, 1.0f, GridLineColor());
        DrawTextUIBold(item.name, x + S(20), y, 13, TextColor());
        DrawTextUI(item.rule, x + contentW - S(96), y, 13, MutedTextColor());
        y += S(20);
    }
    y += S(4);
    DrawTextUI("Agent rules: " + MovementProfileName(), x, y, 13, MutedTextColor());
    y += S(20);
    std::string rule;
    ...
    DrawTextUI(rule, x, y, 13, MutedTextColor());
    y += S(24);
}
...
void App::DrawMapCanvas() {
    ...
    if (frame != nullptr && frame->path.size() >= 2) {
        ...
        const Color core = themeMode_ == ThemeMode::Dark ? Color{ 255, 209, 139, 248 } : Color{ 238, 91, 64, 248 };
        const Color outline = themeMode_ == ThemeMode::Dark ? Color{ 38, 28, 50, 185 } : Color{ 255, 247, 230, 210 };
        if (smoothPath_ && frame->path.size() >= 3) {
            points = RoundedPolyline2D(points, std::clamp(cellSize * 0.46f, 7.0f, 28.0f), 10);
        }
        DrawPathLine2D(points, width, outline, core);
    }

    DrawAgentComparison2D(startX, startY, cellSize);
...
void App::DrawAgentComparison2D(float startX, float startY, float cellSize) const {
    if (!showAgentComparison_ || agentComparisonRows_.empty()) return;
    auto centerOf = [&](int idx) {
        GridCoord gc = map_.Coord(idx);
        return Vector2{ startX + (static_cast<float>(gc.col) + 0.5f) * cellSize,
                        startY + (static_cast<float>(gc.row) + 0.5f) * cellSize };
    ...
    const Color outline = themeMode_ == ThemeMode::Dark ? Color{ 20, 18, 32, 170 } : Color{ 255, 250, 236, 190 };
    for (const auto& row : agentComparisonRows_) {
        if (!row.found || row.path.size() < 2) continue;
        std::vector<Vector2> points;
        points.reserve(row.path.size());
        for (int cell : row.path) points.push_back(centerOf(cell));
        if (smoothPath_ && points.size() >= 3) points = RoundedPolyline2D(points, std::clamp(cellSize * 0.40f, 6.0f, 24.0f), 8);
        DrawPathLine2D(points, width, outline, ProfileColor(row.profile, themeMode_));
    }
}

void App::DrawAgentComparison3D() const {
    if (!showAgentComparison_ || agentComparisonRows_.empty()) return;
    for (const auto& row : agentComparisonRows_) {
        if (!row.found || row.path.size() < 2) continue;
        DrawPath3DColored(row.path, ProfileColor(row.profile, themeMode_), 0.046f);
    }
}
...
void App::DrawTerrain3D() {
    ...
    if (!smoothTerrain3D_ && map_.Count() <= 9000) {
    ...
    DrawRectangleLinesEx(canvas_, 1.0f, GridLineColor());
    DrawTextUI(smoothTerrain3D_ ? "3D diorama presentation" : "3D editor mode", canvas_.x + S(22.0f), canvas_.y + S(22.0f), 19, TextColor());
    DrawTextUI(smoothTerrain3D_ ? "Diorama hides the dense grid and turns the map into a clean terrain sandbox." : "Editor mode keeps every cell visible for precise editing and picking.", canvas_.x + S(22.0f), canvas_.y + S(49.0f), 14, MutedTextColor());
...
void App::DrawSmoothTerrain3D(const SearchFrame* frame) const {
    // v1.0 Diorama mode. This is no longer a second editor surface. The goal is
    // presentation: hide the dense grid, keep semantic layers readable, and show
    // obstacles as buildings, roads as ribbons, water as recessed plates, and the
    // computed route as a navigation tube. The algorithm still runs on the grid.

    const float worldW = static_cast<float>(map_.Cols());
    const float worldH = static_cast<float>(map_.Rows());
    const Color baseTop = themeMode_ == ThemeMode::Dark ? Color{ 52, 42, 72, 255 } : Color{ 252, 222, 207, 255 };
    const Color baseSide = themeMode_ == ThemeMode::Dark ? Color{ 31, 27, 45, 255 } : Color{ 214, 174, 158, 255 };
    const Color boardTrim = themeMode_ == ThemeMode::Dark ? Color{ 233, 205, 255, 70 } : Color{ 94, 66, 86, 75 };

    // A solid board gives the view physical weight and avoids the failed mesh/relief look.
    ...
    auto pFor = [&](int idx, float lift = 0.06f) {
        return CellWorldPosition(idx, lift);
    };

    auto isFrameCell = [&](int idx, const std::vector<int>& list) {
        return frame != nullptr && ContainsCell(list, idx);
    };

    auto overlayFor = [&](int idx) -> Color {
        if (frame == nullptr) return Color{ 0, 0, 0, 0 };
        if (ContainsCell(frame->path, idx)) return themeMode_ == ThemeMode::Dark ? Color{ 255, 205, 132, 185 } : Color{ 238, 91, 64, 172 };
        if (frame->current == idx) return themeMode_ == ThemeMode::Dark ? Color{ 255, 136, 112, 185 } : Color{ 231, 82, 62, 172 };
        if (ContainsCell(frame->open, idx)) return themeMode_ == ThemeMode::Dark ? Color{ 92, 208, 224, 110 } : Color{ 67, 178, 199, 98 };
        if (ContainsCell(frame->closed, idx)) return themeMode_ == ThemeMode::Dark ? Color{ 122, 111, 158, 90 } : Color{ 156, 136, 186, 82 };
        return Color{ 0, 0, 0, 0 };
    };

    // Terrain semantic layer. In diorama mode we deliberately do NOT draw every
    // plain cell. Only meaningful terrain types become visual objects.
    for (int idx = 0; idx < map_.Count(); ++idx) {
        const Cell& cell = map_.GetCell(idx);
        Vector3 p = pFor(idx, 0.052f);

        if (cell.terrain == TerrainType::Water) {
            Color water = themeMode_ == ThemeMode::Dark ? Color{ 85, 177, 222, 210 } : Color{ 98, 188, 226, 198 };
            DrawFlatShadedCubeV({ p.x, 0.075f, p.z }, { 0.96f, 0.030f, 0.96f }, water);
            continue;
        }

        if (cell.terrain == TerrainType::Road) {
            Color road = themeMode_ == ThemeMode::Dark ? Color{ 255, 180, 112, 225 } : Color{ 235, 143, 101, 220 };
            Color stripe = themeMode_ == ThemeMode::Dark ? Color{ 255, 236, 184, 175 } : Color{ 255, 244, 215, 170 };
            DrawFlatShadedCubeV({ p.x, 0.125f, p.z }, { 0.94f, 0.045f, 0.94f }, road);

            GridCoord g = map_.Coord(idx);
            // Draw a subtle stripe in the direction where the road continues.
            bool horizontal = false;
            bool vertical = false;
            if (g.col > 0 && map_.GetCell(map_.Index(g.row, g.col - 1)).terrain == TerrainType::Road) horizontal = true;
            if (g.col + 1 < map_.Cols() && map_.GetCell(map_.Index(g.row, g.col + 1)).terrain == TerrainType::Road) horizontal = true;
            if (g.row > 0 && map_.GetCell(map_.Index(g.row - 1, g.col)).terrain == TerrainType::Road) vertical = true;
            if (g.row + 1 < map_.Rows() && map_.GetCell(map_.Index(g.row + 1, g.col)).terrain == TerrainType::Road) vertical = true;
        ...
        if (cell.terrain == TerrainType::Forest) {
            // Forest becomes sparse decorative trees, not a solid color tile layer.
            GridCoord g = map_.Coord(idx);
            if (((idx * 31 + g.row * 17 + g.col * 13) % 11) == 0) {
                Color trunk = themeMode_ == ThemeMode::Dark ? Color{ 157, 104, 73, 255 } : Color{ 119, 82, 56, 255 };
                Color canopy = themeMode_ == ThemeMode::Dark ? Color{ 116, 218, 162, 242 } : Color{ 74, 157, 112, 236 };
        ...
        if (cell.terrain == TerrainType::Mountain) {
            // Mountains become a few larger sculptural rocks. The height value still
            // affects placement but not every cell becomes a noisy triangle.
            GridCoord g = map_.Coord(idx);
            if (((idx * 23 + g.row * 11 + g.col * 19) % 13) == 0) {
                Color rock = themeMode_ == ThemeMode::Dark ? Color{ 210, 190, 236, 238 } : Color{ 143, 119, 160, 222 };
                Color cap = themeMode_ == ThemeMode::Dark ? Color{ 242, 226, 255, 232 } : Color{ 197, 170, 212, 210 };
    ...
    // Obstacles become buildings/walls. This gives diorama mode a different use:
    // it reads like a navigation sandbox rather than a grid editor.
    for (int idx = 0; idx < map_.Count(); ++idx) {
        if (!map_.IsObstacle(idx) || map_.IsWater(idx)) continue;
        const Cell& cell = map_.GetCell(idx);
        GridCoord g = map_.Coord(idx);
        Vector3 p = pFor(idx, 0.0f);
        float h = 0.58f + 0.055f * static_cast<float>((cell.height + g.row + g.col) % 5);
        Color building = themeMode_ == ThemeMode::Dark ? Color{ 83, 75, 104, 255 } : Color{ 164, 149, 160, 255 };
        Color roof = themeMode_ == ThemeMode::Dark ? Color{ 110, 96, 137, 255 } : Color{ 190, 173, 185, 255 };
        DrawShadedCubeV({ p.x, h * 0.5f + 0.08f, p.z }, { 0.70f, h, 0.70f }, building);
        DrawFlatShadedCubeV({ p.x, h + 0.097f, p.z }, { 0.74f, 0.035f, 0.74f }, roof);
    }

    // Algorithm search overlay: large translucent plates, not noisy full-grid recoloring.
    if (frame != nullptr) {
        const int stride = std::max(1, map_.Count() / 2400);
        for (int idx = 0; idx < map_.Count(); idx += stride) {
...
void App::DrawPath3D(const std::vector<int>& path) const {
    const Color core = themeMode_ == ThemeMode::Dark ? Color{ 255, 209, 139, 255 } : Color{ 238, 91, 64, 255 };
    DrawPath3DColored(path, core, 0.065f);
}

void App::DrawPath3DColored(const std::vector<int>& path, Color core, float radius) const {
    if (path.size() < 2) return;

    std::vector<Vector3> points;
    points.reserve(path.size());
    for (int cell : path) points.push_back(CellWorldPosition(cell, 0.66f));

    if (smoothPath_ && path.size() >= 3) {
        points = RoundedPolyline3D(points, 0.46f, 10);
    }

    const Color outline = themeMode_ == ThemeMode::Dark ? Color{ 28, 22, 42, 210 } : Color{ 255, 247, 230, 225 };
...
void App::DrawCellSelection3D(int cell, Color color) const {
    ...
    DrawFlatShadedCubeV(center, { size.x, 0.012f, size.z }, Color{ color.r, color.g, color.b, 42 });
}
...
void App::HandleMapEditing() {
    ...
    if (view3D_) {
        ...
        if (IsMouseButtonDown(MOUSE_LEFT_BUTTON)) {
            ...
            if (IsKeyDown(KEY_LEFT_SHIFT) || IsKeyDown(KEY_RIGHT_SHIFT)) {
                if (lastEditedCell_ != hoveredCell3D_) PushUndoState("Set start");
                lastEditedCell_ = hoveredCell3D_;
                map_.SetStart(hoveredCell3D_);
                ResetVisualization();
                return;
            }
            if (IsKeyDown(KEY_LEFT_CONTROL) || IsKeyDown(KEY_RIGHT_CONTROL)) {
                if (lastEditedCell_ != hoveredCell3D_) PushUndoState("Set goal");
                lastEditedCell_ = hoveredCell3D_;
                map_.SetGoal(hoveredCell3D_);
                ResetVisualization();
                return;
            }
            if (lastEditedCell_ != hoveredCell3D_) {
                PushUndoState("Edit cell");
                lastEditedCell_ = hoveredCell3D_;
    ...
    if (IsMouseButtonDown(MOUSE_LEFT_BUTTON)) {
        selectedCell_ = cell;
        if (IsKeyDown(KEY_LEFT_SHIFT) || IsKeyDown(KEY_RIGHT_SHIFT)) {
            if (lastEditedCell_ != cell) PushUndoState("Set start");
            lastEditedCell_ = cell;
            map_.SetStart(cell);
            ResetVisualization();
            return;
        }
        if (IsKeyDown(KEY_LEFT_CONTROL) || IsKeyDown(KEY_RIGHT_CONTROL)) {
            if (lastEditedCell_ != cell) PushUndoState("Set goal");
            lastEditedCell_ = cell;
            map_.SetGoal(cell);
            ResetVisualization();
            return;
        }
        if (lastEditedCell_ != cell) {
            PushUndoState("Edit cell");
            lastEditedCell_ = cell;
...
void App::GenerateRandomMap() {
    ...
    hoveredCell3D_ = -1;
    ClearHistory();
    showAgentComparison_ = false;
    agentComparisonRows_.clear();
    ResetVisualization();
...
void App::GenerateTerrainMap() {
    ...
    agentComparisonRows_.clear();
    ResetVisualization();
    statusMessage_ = "Terrain map generated: hills, road, river, forest and mountains";
}
...
void App::GenerateCityPreset() {
    ...
    for (int r = 0; r < map_.Rows(); ++r) {
        for (int c = 0; c < map_.Cols(); ++c) {
            int idx = map_.Index(r, c);
            map_.SetHeight(idx, heightDist(rng));
            if (r % 6 == 2 || c % 9 == 4) {
                map_.SetTerrain(idx, TerrainType::Road);
                map_.SetObstacle(idx, false);
                map_.SetHeight(idx, 1);
            } else if (((r / 3 + c / 4) % 5 == 0) && (r % 6 != 0) && (c % 9 != 0)) {
                map_.SetObstacle(idx, true);
                map_.SetTerrain(idx, TerrainType::Plain);
                map_.SetHeight(idx, 2 + ((r + c) % 5));
            } else if ((r + c * 2) % 19 == 0) {
                map_.SetTerrain(idx, TerrainType::Forest);
            }
        }
    }
    map_.SetStart(map_.Index(std::min(2, map_.Rows() - 1), std::min(2, map_.Cols() - 1)));
    map_.SetGoal(map_.Index(std::max(0, map_.Rows() - 3), std::max(0, map_.Cols() - 3)));
    selectedCell_ = -1;
    hoveredCell3D_ = -1;
    ClearHistory();
    showAgentComparison_ = false;
    ...
    ResetVisualization();
    ResetCamera3D();
    statusMessage_ = "City preset generated: roads, buildings and blocks";
}

void App::GenerateRiverPreset() {
    map_.ClearAll();
    ...
    for (int r = 0; r < rows; ++r) {
        for (int c = 0; c < cols; ++c) {
            int idx = map_.Index(r, c);
            map_.SetHeight(idx, std::clamp(4 + static_cast<int>(2.2 * std::sin(c * 0.23) + 1.4 * std::cos(r * 0.31)), 0, 9));
            if ((r + c) % 17 == 0) map_.SetTerrain(idx, TerrainType::Forest);
        }
    }
    for (int c = 0; c < cols; ++c) {
        ...
        for (int dr = -2; dr <= 2; ++dr) {
            int r = center + dr;
            if (!map_.InBounds(r, c)) continue;
            int idx = map_.Index(r, c);
            map_.SetObstacle(idx, false);
            map_.SetTerrain(idx, TerrainType::Water);
            map_.SetHeight(idx, 0);
        }
    }
    ...
    map_.SetStart(map_.Index(rows / 2, 1));
    map_.SetGoal(map_.Index(rows / 2, cols - 2));
    selectedCell_ = -1;
    hoveredCell3D_ = -1;
    ClearHistory();
    showAgentComparison_ = false;
    ...
    ResetVisualization();
    ResetCamera3D();
    statusMessage_ = "River crossing preset generated: bridges create route choices";
}

void App::GenerateMountainPreset() {
    map_.ClearAll();
    const int rows = map_.Rows();
    const int cols = map_.Cols();
    for (int r = 0; r < rows; ++r) {
        for (int c = 0; c < cols; ++c) {
            ...
            map_.SetHeight(idx, h);
            if (h >= 5) map_.SetTerrain(idx, TerrainType::Mountain);
            if (h >= 8 && (c < cols / 3 || c > cols * 2 / 3)) map_.SetObstacle(idx, true);
            if ((r + c) % 21 == 0) map_.SetTerrain(idx, TerrainType::Forest);
        }
    }
    // Two low passes through the ridge.
    for (int passCol : { cols / 3, cols * 2 / 3 }) {
        for (int c = std::max(0, passCol - 2); c <= std::min(cols - 1, passCol + 2); ++c) {
            for (int r = rows / 2 - 4; r <= rows / 2 + 4; ++r) {
                ...
                map_.SetObstacle(idx, false);
                map_.SetTerrain(idx, TerrainType::Road);
                map_.SetHeight(idx, 2);
            }
        }
    }
    map_.SetStart(map_.Index(rows - 3, 2));
    map_.SetGoal(map_.Index(2, cols - 3));
    selectedCell_ = -1;
    hoveredCell3D_ = -1;
    ClearHistory();
    showAgentComparison_ = false;
    ...
    ResetVisualization();
    ResetCamera3D();
    statusMessage_ = "Mountain pass preset generated: height cost matters";
}

void App::RunDemoMode() {
    GenerateRiverPreset();
    view3D_ = true;
    smoothTerrain3D_ = true;
    movementProfile_ = MovementProfile::Pedestrian;
    options_.allowDiagonal = true;
    heuristicWeightValue_ = 1.25f;
    terrainWeightValue_ = 1.0f;
    heightWeightValue_ = 0.35f;
    ResetCamera3D();
    RunAlgorithm(AlgorithmKind::AStar);
    frameIndex_ = 0;
    playing_ = true;
    statusMessage_ = "Demo mode: River preset + Diorama + A* animation";
}

void App::ApplyMapSize() {
    int rows = static_cast<int>(std::round(mapRowsValue_));
    int cols = static_cast<int>(std::round(mapColsValue_));
    PushUndoState("Resize map");
...
void App::CompareAgents() {
    SyncOptionsFromSliders();
    agentComparisonRows_.clear();
    const std::vector<MovementProfile> profiles = {
    ...
    for (MovementProfile profile : profiles) {
        AlgorithmOptions opts = options_;
        opts.profile = profile;
        SearchResult r = Pathfinder::Run(map_, AlgorithmKind::AStar, opts);
        agentComparisonRows_.push_back({ profile, ProfileDisplayName(profile), r.found, r.pathLength, r.expandedNodes, r.elapsedMs, r.finalPath });
    }
    showAgentComparison_ = true;
    statusMessage_ = "Agent path comparison finished: same map, different cost models";
}

void App::ExportScreenshot() {
    std::string path = EnsurePngExtension(NativePngFileDialog());
    if (path.empty()) {
        statusMessage_ = "Screenshot export canceled";
        return;
    }
    try {
        std::filesystem::path p(path);
        ...
        statusMessage_ = "Screenshot exported: " + CompactDisplayPath(path);
    } catch (...) {
        statusMessage_ = "Failed to export screenshot";
    }
}
...
void App::PushUndoState(const std::string& label) {
    undoStack_.push_back(MapHistoryState{ map_, label });
    if (undoStack_.size() > 80) undoStack_.erase(undoStack_.begin());
    redoStack_.clear();
}
...
void App::UndoEdit() {
    ...
    redoStack_.push_back(MapHistoryState{ map_, "redo" });
    MapHistoryState state = undoStack_.back();
    undoStack_.pop_back();
    map_ = state.map;
    mapRowsValue_ = static_cast<float>(map_.Rows());
    mapColsValue_ = static_cast<float>(map_.Cols());
    ...
    agentComparisonRows_.clear();
    ResetVisualization();
    statusMessage_ = "Undo: " + state.label;
}
...
void App::RedoEdit() {
    ...
    undoStack_.push_back(MapHistoryState{ map_, "undo" });
    MapHistoryState state = redoStack_.back();
    redoStack_.pop_back();
    map_ = state.map;
    mapRowsValue_ = static_cast<float>(map_.Rows());
    mapColsValue_ = static_cast<float>(map_.Cols());
...
BoundingBox App::CellBoundingBox3D(int cell) const {
    ...
    const float h = CellVisualHeight3D(cell);
...
float App::SoftSurfaceHeightAt(int row, int col) const {
    row = std::clamp(row, 0, map_.Rows() - 1);
    col = std::clamp(col, 0, map_.Cols() - 1);

    // Diorama height is intentionally subtle. It is used to lift route markers,
    // start/goal, and selection above the board; the visual terrain itself is
    // represented by semantic layers rather than a noisy height mesh.
    float sum = 0.0f;
    ...
    for (int dr = -1; dr <= 1; ++dr) {
        for (int dc = -1; dc <= 1; ++dc) {
            ...
            float h = static_cast<float>(map_.GetCell(idx).height) / 9.0f;
            if (map_.IsWater(idx)) h *= 0.12f;
    ...
    const float scale = std::max(0.10f, terrainHeightScale_) * 0.85f;
    return 0.09f + h01 * scale;
}

float App::CellVisualHeight3D(int cell) const {
    if (cell < 0 || cell >= map_.Count()) return 0.0f;
    if (!smoothTerrain3D_) return CellSurfaceHeight(cell);
    GridCoord g = map_.Coord(cell);
    return SoftSurfaceHeightAt(g.row, g.col);
}
...
Vector3 App::CellWorldPosition(int cell, float extraHeight) const {
    ...
    const float x = static_cast<float>(g.col) - static_cast<float>(map_.Cols() - 1) * 0.5f;
    const float z = static_cast<float>(g.row) - static_cast<float>(map_.Rows() - 1) * 0.5f;
    return { x, CellVisualHeight3D(cell) + extraHeight, z };
}
...
Vector3 App::TerrainVertexWorld(int row, int col, float extraHeight) const {
    ...
    if (smoothTerrain3D_) {
        return { x, SoftSurfaceHeightAt(row, col) + extraHeight, z };
    }
