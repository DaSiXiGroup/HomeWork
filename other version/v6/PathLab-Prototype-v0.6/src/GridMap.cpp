...
double GridMap::StepCost(int from, int to, double terrainWeight, double heightWeight) const {
    ...
    switch (cells_[static_cast<size_t>(to)].terrain) {
        case TerrainType::Road:
            // Roads are not lower altitude. They are better infrastructure:
            // lower distance cost and reduced slope penalty.
            moveMultiplier = 0.62;
            slopeMultiplier = 0.35;
            break;
        case TerrainType::Forest:
            // Forest is passable but slow due to vegetation density.
            moveMultiplier = 1.18;
            slopeMultiplier = 1.05;
            break;
        case TerrainType::Mountain:
            // Mountain terrain means rough rock surface; actual elevation is still `height`.
            moveMultiplier = 1.32;
    ...
    const double terrain = terrainWeight * TerrainCost(to);
    ...
    return std::max(0.05, base * moveMultiplier + terrain + height);
}
