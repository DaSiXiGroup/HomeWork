...
SearchResult Pathfinder::Run(const GridMap& map, AlgorithmKind algorithm, const AlgorithmOptions& options) {
    ...
    if (algorithm == AlgorithmKind::BFS) {
        ...
        while (!q.empty()) {
            ...
            for (int next : map.Neighbors(current, options.allowDiagonal)) {
        ...
        while (!pq.empty()) {
            ...
            for (int next : map.Neighbors(current, options.allowDiagonal)) {
                ...
                double stepCost = map.StepCost(current, next, options.terrainWeight, options.heightWeight);
...
double Pathfinder::ComputePathCost(const GridMap& map, const std::vector<int>& path, const AlgorithmOptions& options, bool forceUnitCost) {
    ...
    for (size_t i = 1; i < path.size(); ++i) {
        if (forceUnitCost) {
            ...
            total += map.StepCost(path[i - 1], path[i], options.terrainWeight, options.heightWeight);
        }
    }
    return total;
}
