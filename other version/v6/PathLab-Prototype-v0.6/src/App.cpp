...
void App::Run() {
    ...
    InitWindow(kWindowWidth, kWindowHeight, "PathLab Prototype v0.6 - 3D terrain picking and editing");
...
void App::DrawTopBar() {
    ...
    DrawTextUI("PathLab Prototype v0.6", S(20), S(15), 26, TextColor());
    DrawTextUI("large-screen path planning visualization platform", S(255), S(22), 17, MutedTextColor());
...
void App::DrawLeftPanel() {
    ...
    if (ToggleButton({ x, y, S(300), S(34) }, options_.allowDiagonal ? "Diagonal: ON" : "Diagonal: OFF", options_.allowDiagonal)) {
...
void App::DrawBottomPanel() {
    ...
    DrawTextUI(view3D_ ? "3D: left click select/edit | right/middle rotate | wheel zoom | WASD pan | R reset | F5 2D" : "2D: Left paint | Right erase | Shift+Left start | Ctrl+Left goal | F5 3D", 
...
void App::DrawTerrain3D() {
    ...
    const float worldW = static_cast<float>(map_.Cols()) * cellScale;
    const float worldH = static_cast<float>(map_.Rows()) * cellScale;
    ...
    // Draw terrain-specific visual features after base blocks. Elevation is still independent;
    // these markers express movement semantics rather than height.
    if (map_.Count() <= 9000) {
    ...
    if (frame != nullptr && frame->path.size() >= 2) {
        for (size_t i = 1; i < frame->path.size(); ++i) {
            Vector3 a = CellWorldPosition(frame->path[i - 1], 0.48f);
            Vector3 b = CellWorldPosition(frame->path[i], 0.48f);
            DrawLine3D(a, b, Color{ 255, 238, 96, 255 });
            DrawSphere(a, 0.105f, Color{ 255, 238, 96, 255 });
        }
        DrawSphere(CellWorldPosition(frame->path.back(), 0.48f), 0.105f, Color{ 255, 238, 96, 255 });
    }
    ...
    DrawTextUI("3D terrain editor", canvas_.x + S(22.0f), canvas_.y + S(22.0f), 20, TextColor());
    DrawTextUI("Left click selects or applies current tool. Right/middle drag rotates camera.", canvas_.x + S(22.0f), canvas_.y + S(50.0f), 15, MutedTextColor());

    if (hoveredCell3D_ >= 0) {
        DrawTextUI("Hover: " + CellInfo(hoveredCell3D_), canvas_.x + S(22.0f), canvas_.y + S(76.0f), 15, TextColor());
    }
    if (selectedCell_ >= 0) {
        DrawTextUI("Selected: " + CellInfo(selectedCell_), canvas_.x + S(22.0f), canvas_.y + S(101.0f), 15, AccentColor());
    }
}
...
std::string App::CellInfo(int cell) const {
    ...
    GridCoord g = map_.Coord(cell);
    const Cell& c = map_.GetCell(cell);
    std::string type = map_.IsWalkable(cell) ? "walkable" : "blocked";
