...
void App::Run() {
    ...
    InitWindow(kWindowWidth, kWindowHeight, "PathLab Prototype v0.8.3 - clearer UI + map slots");
...
void App::DrawLeftPanel() {
    ...
    auto section = [&](const char* title) {
        DrawTextUIBold(title, x, y, 19, TextColor());
    ...
    y += S(40);
    if (ToggleButton({ x, y, bw, S(34) }, smoothTerrain3D_ ? "Surface: soft" : "Surface: solid", smoothTerrain3D_)) smoothTerrain3D_ = !smoothTerrain3D_;
...
void App::DrawRightPanel() {
    ...
    section("OUTPUT");
    if (ToggleButton({ x, y, bw, gh }, "Slot 1", selectedMapSlot_ == 1)) selectedMapSlot_ = 1;
    if (ToggleButton({ x + bw + gap, y, bw, gh }, "Slot 2", selectedMapSlot_ == 2)) selectedMapSlot_ = 2;
    if (ToggleButton({ x + (bw + gap) * 2.0f, y, bw, gh }, "Slot 3", selectedMapSlot_ == 3)) selectedMapSlot_ = 3;
    y += S(40);
    ...
    y += S(38);
    DrawTextUI("File: " + SelectedMapPath(), x, y, 13, MutedTextColor());
    y += S(34);
...
void App::DrawSmoothTerrain3D(const SearchFrame* frame) const {
    ...
    auto softHeightAt = [&](int row, int col) {
        ...
        return 0.06f + (sum / static_cast<float>(count)) * terrainHeightScale_ * 1.55f;
    };
    ...
    auto heightColor = [&](float h, float shade) {
        ...
        Color low = themeMode_ == ThemeMode::Dark ? Color{ 54, 49, 75, 255 } : Color{ 246, 218, 202, 255 };
        Color mid = themeMode_ == ThemeMode::Dark ? Color{ 90, 77, 118, 255 } : Color{ 224, 194, 178, 255 };
        Color high = themeMode_ == ThemeMode::Dark ? Color{ 214, 181, 235, 255 } : Color{ 165, 145, 184, 255 };
        Color base = h < 0.55f ? Blend(low, mid, h / 0.55f) : Blend(mid, high, (h - 0.55f) / 0.45f);
        Color light = themeMode_ == ThemeMode::Dark ? Color{ 255, 230, 255, base.a } : Color{ 255, 246, 236, base.a };
        Color dark = themeMode_ == ThemeMode::Dark ? Color{ 26, 24, 43, base.a } : Color{ 150, 118, 126, base.a };
        if (shade >= 0.0f) return Blend(base, light, std::clamp(shade, 0.0f, 0.28f));
        return Blend(base, dark, std::clamp(-shade, 0.0f, 0.32f));
    };
    ...
    for (int r = 0; r < map_.Rows() - 1; ++r) {
        for (int c = 0; c < map_.Cols() - 1; ++c) {
            ...
            float shade = std::clamp((-dx * 0.55f + dz * 0.35f), -0.32f, 0.28f);
    ...
    Color contour = themeMode_ == ThemeMode::Dark ? Color{ 255, 235, 255, 54 } : Color{ 105, 82, 100, 48 };
    const int step = std::max(4, std::max(map_.Rows(), map_.Cols()) / 14);
...
void App::SaveCurrentMap() {
    try {
        ...
        statusMessage_ = "Failed to create maps directory";
        return;
    }

    const std::string path = SelectedMapPath();
    if (map_.SaveToFile(path)) {
        statusMessage_ = "Map saved to " + path;
    } else {
        statusMessage_ = "Failed to save " + path;
    }
}
...
void App::LoadCurrentMap() {
    ...
    if (map_.LoadFromFile(path, &error)) {
        ...
        hoveredCell3D_ = -1;
        ResetVisualization();
        statusMessage_ = "Map loaded from " + path;
    } else {
        statusMessage_ = error.empty() ? ("Failed to load " + path) : error;
    }
}
...
bool App::Button(Rectangle bounds, const char* text) {
    ...
    DrawTextUIBold(text, bounds.x + (bounds.width - textWidth) / 2.0f, bounds.y + (bounds.height - S(20.0f)) * 0.5f, 16, TextColor());
    return pressed;
}
...
bool App::ToggleButton(Rectangle bounds, const char* text, bool active) {
    ...
    float textWidth = MeasureTextUI(text, 16);
    DrawTextUIBold(text, bounds.x + (bounds.width - textWidth) / 2.0f, bounds.y + (bounds.height - S(20.0f)) * 0.5f, 16, active ? Color{255, 248, 235, 255} : TextColor());
    return pressed;
}

bool App::Slider(Rectangle bounds, const char* label, float* value, float minValue, float maxValue, bool integerDisplay) {
    DrawTextUIBold(label, bounds.x, bounds.y - S(20.0f), 14, TextColor());
    ...
    DrawTextUI(buffer, bounds.x + bounds.width - S(72.0f), bounds.y - S(20.0f), 14, MutedTextColor());
...
void App::DrawTextUI(const std::string& text, float x, float y, float fontSize, Color color) const {
    ...
    DrawTextEx(uiFont_, text.c_str(), Vector2{ x, y }, scaledSize, 0.65f * uiScale_, color);
}

void App::DrawTextUIBold(const std::string& text, float x, float y, float fontSize, Color color) const {
    const float scaledSize = fontSize * uiScale_;
    const float spacing = 0.65f * uiScale_;
    DrawTextEx(uiFont_, text.c_str(), Vector2{ x, y }, scaledSize, spacing, color);
    DrawTextEx(uiFont_, text.c_str(), Vector2{ x + std::max(0.45f, 0.45f * uiScale_), y }, scaledSize, spacing, color);
}

float App::MeasureTextUI(const std::string& text, float fontSize) const {
    const float scaledSize = fontSize * uiScale_;
    return MeasureTextEx(uiFont_, text.c_str(), scaledSize, 0.65f * uiScale_).x;
}
