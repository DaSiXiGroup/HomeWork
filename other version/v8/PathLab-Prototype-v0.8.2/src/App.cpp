...
void App::Run() {
    ...
    InitWindow(kWindowWidth, kWindowHeight, "PathLab Prototype v0.8.2 - readable mesh + zoomable 2D editor");
...
void App::Layout() {
    ...
    const float topH = S(58.0f);
    const float bottomH = S(92.0f);
    const float leftW = S(330.0f);
    const float rightW = S(380.0f);
...
void App::Update() {
    ...
    if (view3D_) {
        UpdateCamera3D();
    } else if (CheckCollisionPointRec(GetMousePosition(), canvas_)) {
...
void App::DrawTopBar() {
    DrawRectangle(0, 0, GetScreenWidth(), static_cast<int>(S(58.0f)), DarkPanelColor());
    DrawTextUI("PathLab", S(20), S(13), 25, TextColor());
    DrawTextUI("v0.8.2", S(115), S(20), 15, MutedTextColor());

    const float buttonW = S(124.0f);
    const float buttonH = S(32.0f);
    const float gap = S(12.0f);
    float cx = static_cast<float>(GetScreenWidth()) * 0.5f - (buttonW * 3.0f + gap * 2.0f) * 0.5f;
    if (Button({ cx, S(13.0f), buttonW, buttonH }, view3D_ ? "View: 3D" : "View: 2D")) {
        ToggleViewMode();
    }
    if (Button({ cx + buttonW + gap, S(13.0f), buttonW, buttonH }, themeMode_ == ThemeMode::Dark ? "Theme: Dark" : "Theme: Light")) {
        ToggleThemeMode();
    }
    if (Button({ cx + (buttonW + gap) * 2.0f, S(13.0f), buttonW, buttonH }, view3D_ ? "Reset 3D" : "Reset 2D")) {
        if (view3D_) {
            ResetCamera3D();
    ...
    DrawTextUI(shortcuts, static_cast<float>(GetScreenWidth()) - S(560.0f), S(20.0f), 14, MutedTextColor());
}
...
void App::DrawLeftPanel() {
    ...
    float x = leftPanel_.x + S(20.0f);
    float y = leftPanel_.y + S(16.0f);
    const float contentW = leftPanel_.width - S(40.0f);
    const float gap = S(10.0f);
    const float bw = (contentW - gap) * 0.5f;

    auto section = [&](const char* title) {
        DrawTextUI(title, x, y, 18, TextColor());
        DrawRectangleRec({ x, y + S(25.0f), contentW, 1.0f }, GridLineColor());
        y += S(36.0f);
    };

    section("MAP");
    if (Button({ x, y, bw, S(30) }, "Random")) GenerateRandomMap();
    if (Button({ x + bw + gap, y, bw, S(30) }, "Terrain")) GenerateTerrainMap();
    y += S(36);
    if (Button({ x, y, bw, S(30) }, "Maze")) GenerateMazeMap();
    if (Button({ x + bw + gap, y, bw, S(30) }, "Clear")) {
        map_.ClearAll();
        ResetVisualization();
    ...
    y += S(36);
    if (Button({ x, y, contentW, S(30) }, "Apply map size")) ApplyMapSize();
    y += S(42);

    section("ALGORITHMS");
    if (Button({ x, y, bw, S(30) }, "BFS")) RunAlgorithm(AlgorithmKind::BFS);
    if (Button({ x + bw + gap, y, bw, S(30) }, "Dijkstra")) RunAlgorithm(AlgorithmKind::Dijkstra);
    y += S(36);
    if (Button({ x, y, bw, S(30) }, "A*")) RunAlgorithm(AlgorithmKind::AStar);
    if (Button({ x + bw + gap, y, bw, S(30) }, "Weighted A*")) RunAlgorithm(AlgorithmKind::WeightedAStar);
    y += S(36);
    if (Button({ x, y, bw, S(30) }, "Greedy")) RunAlgorithm(AlgorithmKind::GreedyBestFirst);
    if (Button({ x + bw + gap, y, bw, S(30) }, "Benchmark")) RunBenchmark();
    y += S(42);

    section("AGENT & COST");
    if (ToggleButton({ x, y, contentW, S(30) }, options_.allowDiagonal ? "Diagonal movement: ON" : "Diagonal movement: OFF", options_.allowDiagonal)) {
    ...
    if (ToggleButton({ x + bw + gap, y, bw, S(30) }, "Car", movementProfile_ == MovementProfile::Car)) movementProfile_ = MovementProfile::Car;
    y += S(36);
    if (ToggleButton({ x, y, bw, S(30) }, "Offroad", movementProfile_ == MovementProfile::Offroad)) movementProfile_ = MovementProfile::Offroad;
    if (ToggleButton({ x + bw + gap, y, bw, S(30) }, "Drone", movementProfile_ == MovementProfile::Drone)) movementProfile_ = MovementProfile::Drone;
    y += S(38);
    ...
    y += S(36);
    if (ToggleButton({ x, y, bw, S(30) }, smoothTerrain3D_ ? "Surface: soft" : "Surface: solid", smoothTerrain3D_)) smoothTerrain3D_ = !smoothTerrain3D_;
    if (ToggleButton({ x + bw + gap, y, bw, S(30) }, smoothPath_ ? "Path: curve" : "Path: polyline", smoothPath_)) smoothPath_ = !smoothPath_;
    y += S(36);
    if (ToggleButton({ x, y, contentW, S(30) }, showGraphOverlay_ ? "Algorithm graph: ON" : "Algorithm graph: OFF", showGraphOverlay_)) showGraphOverlay_ = !showGraphOverlay_;
}
...
void App::DrawRightPanel() {
    ...
    section("OUTPUT");
    if (Button({ x, y, bw, gh }, "Save")) SaveCurrentMap();
    if (Button({ x + bw + gap, y, bw, gh }, "Load")) LoadCurrentMap();
    if (Button({ x + (bw + gap) * 2.0f, y, bw, gh }, "CSV")) ExportBenchmarkCsv();
    y += S(42);
...
void App::DrawMapCanvas() {
    ...
    for (int r = 0; r < map_.Rows(); ++r) {
        for (int c = 0; c < map_.Cols(); ++c) {
            ...
            if (cellSize >= 26.0f && !map_.IsObstacle(idx)) {
                DrawTextUI(std::to_string(cell.height), rect.x + cellSize * 0.38f, rect.y + cellSize * 0.28f, 13, MutedTextColor());
            }
        }
    }
...
void App::DrawSmoothTerrain3D(const SearchFrame* frame) const {
    auto heightColor = [&](float h) {
        ...
        Color low = themeMode_ == ThemeMode::Dark ? Color{ 58, 52, 80, 255 } : Color{ 248, 221, 208, 255 };
        Color mid = themeMode_ == ThemeMode::Dark ? Color{ 92, 78, 116, 255 } : Color{ 231, 204, 191, 255 };
        Color high = themeMode_ == ThemeMode::Dark ? Color{ 186, 157, 218, 255 } : Color{ 206, 185, 206, 255 };
        return h < 0.55f ? Blend(low, mid, h / 0.55f) : Blend(mid, high, (h - 0.55f) / 0.45f);
    };

    auto cellHeightNorm = [&](int idx) {
        return static_cast<float>(map_.GetCell(idx).height) / 10.0f;
    };

    // Soft mode intentionally ignores most per-cell terrain colors on the base mesh.
    // Otherwise every quad becomes a different pastel patch and the scene becomes unreadable.
    for (int r = 0; r < map_.Rows() - 1; ++r) {
        for (int c = 0; c < map_.Cols() - 1; ++c) {
            ...
            float h = (cellHeightNorm(idx) + cellHeightNorm(idxRight) + cellHeightNorm(idxDown) + cellHeightNorm(idxDiag)) * 0.25f;
            Color color = heightColor(h);
            ...
            if (anyWater) {
                color = themeMode_ == ThemeMode::Dark ? Color{ 77, 135, 172, 235 } : Color{ 137, 194, 217, 225 };
            } else if (anyRoad) {
                color = Blend(color, themeMode_ == ThemeMode::Dark ? Color{ 208, 145, 118, 255 } : Color{ 231, 190, 170, 255 }, 0.18f);
            }

            // A very small alternating brightness only gives the surface readable relief,
            // not the old hard triangle mosaic.
            Color color2 = Blend(color, themeMode_ == ThemeMode::Dark ? Color{ 28, 24, 42, color.a } : Color{ 255, 246, 238, color.a }, 0.045f);
            DrawTriangle3D(a, b, d, color);
            DrawTriangle3D(b, e, d, color2);
        }
    }

    // Show blocked cells as subtle low caps instead of noisy high square patches.
    for (int idx = 0; idx < map_.Count(); ++idx) {
        if (!map_.IsObstacle(idx) || map_.IsWater(idx)) continue;
        Vector3 p = CellWorldPosition(idx, 0.035f);
        Color cap = themeMode_ == ThemeMode::Dark ? Color{ 52, 47, 68, 190 } : Color{ 160, 147, 156, 165 };
        DrawCubeV({ p.x, p.y, p.z }, { 0.52f, 0.055f, 0.52f }, cap);
    }

    // Algorithm state is drawn as a separate calm overlay so the base terrain stays readable.
    if (frame != nullptr) {
        Color closed = themeMode_ == ThemeMode::Dark ? Color{ 127, 112, 154, 145 } : Color{ 156, 146, 178, 125 };
        Color open = themeMode_ == ThemeMode::Dark ? Color{ 108, 210, 226, 165 } : Color{ 84, 181, 198, 145 };
        Color current = themeMode_ == ThemeMode::Dark ? Color{ 255, 143, 119, 210 } : Color{ 231, 111, 81, 210 };
        auto plate = [&](int cell, Color c) {
            Vector3 p = CellWorldPosition(cell, 0.045f);
            DrawCubeV({ p.x, p.y, p.z }, { 0.56f, 0.035f, 0.56f }, c);
        };
...
void App::LoadUIFont() {
    for (const auto& path : FontCandidates()) {
        ...
        Font loaded = LoadFontEx(path.c_str(), 128, nullptr, 0);
...
bool App::Button(Rectangle bounds, const char* text) {
    ...
    DrawTextUI(text, bounds.x + (bounds.width - textWidth) / 2.0f, bounds.y + (bounds.height - S(18.0f)) * 0.5f, 15, TextColor());
    return pressed;
}
...
bool App::ToggleButton(Rectangle bounds, const char* text, bool active) {
    ...
    DrawTextUI(text, bounds.x + (bounds.width - textWidth) / 2.0f, bounds.y + (bounds.height - S(18.0f)) * 0.5f, 14, active ? Color{255, 248, 235, 255} : TextColor());
    return pressed;
}

bool App::Slider(Rectangle bounds, const char* label, float* value, float minValue, float maxValue, bool integerDisplay) {
    DrawTextUI(label, bounds.x, bounds.y - S(18.0f), 13, TextColor());
    ...
    DrawTextUI(buffer, bounds.x + bounds.width - S(62.0f), bounds.y - S(18.0f), 13, MutedTextColor());
