...
void App::Run() {
    ...
    InitWindow(kWindowWidth, kWindowHeight, "PathLab Prototype v0.8.7 - fixed soft terrain picking");
...
void App::ResetCamera3D() {
    ...
    // Solid view is an editor-like overview. Soft view is a terrain showcase,
    // so it uses a closer, lower camera to reveal height relief instead of
    // collapsing the surface into a flat pastel plane.
    cameraTarget_ = { 0.0f, smoothTerrain3D_ ? 0.75f : 0.0f, 0.0f };
    cameraDistance_ = std::max(smoothTerrain3D_ ? 18.0f : 28.0f, mapSize * (smoothTerrain3D_ ? 0.56f : 0.95f));
    cameraYaw_ = smoothTerrain3D_ ? 0.72f : 0.78f;
    cameraPitch_ = smoothTerrain3D_ ? 0.46f : 0.58f;

    camera_.up = { 0.0f, 1.0f, 0.0f };
    camera_.fovy = smoothTerrain3D_ ? 34.0f : 45.0f;
...
void App::DrawTopBar() {
    ...
    DrawTextUI("v0.8.7", S(128), S(24), 15, MutedTextColor());
...
void App::DrawTerrain3D() {
    ...
    DrawRectangleLinesEx(canvas_, 1.0f, GridLineColor());
    DrawTextUI(smoothTerrain3D_ ? "3D soft heightfield terrain" : "3D solid terrain editor", canvas_.x + S(22.0f), canvas_.y + S(22.0f), 19, TextColor());
    DrawTextUI(smoothTerrain3D_ ? "Soft mode uses one coherent heightfield; picking now uses the same ground projection." : "Solid mode uses cell blocks for precise editing.", canvas_.x + S(22.0f), canvas_.y + S(49.0f), 14, MutedTextColor());
...
void App::DrawSmoothTerrain3D(const SearchFrame* frame) const {
    // v0.8.7: Soft mode uses the same height function for rendering, picking,
    // path placement and selection. The previous version mixed a smoothed
    // surface with raw per-cell bounding boxes, so the ray hit-test could
    // select cells that did not visually match the terrain.

    auto smoothHeight01 = [&](int row, int col) {
        const float display = SoftSurfaceHeightAt(row, col);
        const float scale = std::max(0.42f, terrainHeightScale_) * 2.80f;
        return std::clamp((display - 0.16f) / std::max(0.001f, scale), 0.0f, 1.0f);
    };

    auto displayHeight = [&](int row, int col) {
        return SoftSurfaceHeightAt(row, col);
    };
    ...
    auto vertex = [&](int row, int col, float lift = 0.0f) {
        ...
        return Vector3{ x, displayHeight(row, col) + lift, z };
    };

    auto reliefColor = [&](float h, float shade) {
        h = std::clamp(h, 0.0f, 1.0f);
        // Banded color is deliberate: it reads like contour relief from a
        // distance and avoids the old random-looking triangle patchwork.
        const float banded = std::floor(h * 7.0f) / 7.0f;
        h = Blend(Color{ static_cast<unsigned char>(banded * 255.0f), 0, 0, 255 },
                  Color{ static_cast<unsigned char>(h * 255.0f), 0, 0, 255 }, 0.35f).r / 255.0f;

        Color low{};
        Color mid{};
        Color high{};
        if (themeMode_ == ThemeMode::Dark) {
            low = Color{ 39, 45, 70, 255 };
            mid = Color{ 94, 76, 126, 255 };
            high = Color{ 218, 196, 248, 255 };
        } else {
            low = Color{ 247, 223, 204, 255 };
            mid = Color{ 226, 184, 145, 255 };
            high = Color{ 146, 120, 176, 255 };
        }

        Color base = h < 0.58f ? Blend(low, mid, h / 0.58f) : Blend(mid, high, (h - 0.58f) / 0.42f);
        Color light = themeMode_ == ThemeMode::Dark ? Color{ 255, 239, 255, base.a } : Color{ 255, 249, 235, base.a };
        Color dark = themeMode_ == ThemeMode::Dark ? Color{ 14, 16, 29, base.a } : Color{ 119, 78, 101, base.a };
        if (shade >= 0.0f) return Blend(base, light, std::clamp(shade, 0.0f, 0.52f));
        return Blend(base, dark, std::clamp(-shade, 0.0f, 0.58f));
    };

    // Main terrain surface. Each quad receives a single color so the diagonal
    // split is invisible; the relief comes from height bands and lighting.
    for (int r = 0; r < map_.Rows() - 1; ++r) {
        for (int c = 0; c < map_.Cols() - 1; ++c) {
            ...
            const float h = (smoothHeight01(r, c) + smoothHeight01(r, c + 1)
                + smoothHeight01(r + 1, c) + smoothHeight01(r + 1, c + 1)) * 0.25f;
            const float dx = ((b.y + e.y) - (a.y + d.y)) * 0.5f;
            const float dz = ((d.y + e.y) - (a.y + b.y)) * 0.5f;
            const float shade = std::clamp((-dx * 0.34f + dz * 0.27f), -0.55f, 0.50f);
            Color color = reliefColor(h, shade);
            ...
            const bool anyMountain = map_.GetCell(idx).terrain == TerrainType::Mountain || map_.GetCell(idxRight).terrain == TerrainType::Mountain
                || map_.GetCell(idxDown).terrain == TerrainType::Mountain || map_.GetCell(idxDiag).terrain == TerrainType::Mountain;

            if (anyWater) {
                color = themeMode_ == ThemeMode::Dark ? Color{ 71, 135, 184, 250 } : Color{ 92, 164, 208, 245 };
            } else if (anyRoad) {
                color = Blend(color, themeMode_ == ThemeMode::Dark ? Color{ 239, 168, 116, 255 } : Color{ 218, 141, 103, 255 }, 0.42f);
            } else if (anyForest) {
                color = Blend(color, themeMode_ == ThemeMode::Dark ? Color{ 69, 172, 134, 255 } : Color{ 92, 166, 127, 255 }, 0.30f);
            } else if (anyMountain) {
                color = Blend(color, themeMode_ == ThemeMode::Dark ? Color{ 221, 207, 252, 255 } : Color{ 145, 119, 170, 255 }, 0.22f);
            }
    ...
    // A skirt around the terrain gives the soft surface a readable solid mass.
    Color side = themeMode_ == ThemeMode::Dark ? Color{ 25, 22, 42, 230 } : Color{ 218, 178, 166, 220 };
    const float baseY = -0.06f;
    for (int c = 0; c < map_.Cols() - 1; ++c) {
        Vector3 a = vertex(0, c); Vector3 b = vertex(0, c + 1);
        Vector3 ab{ a.x, baseY, a.z }; Vector3 bb{ b.x, baseY, b.z };
        DrawTriangle3D(a, b, ab, side); DrawTriangle3D(b, bb, ab, side);
        a = vertex(map_.Rows() - 1, c); b = vertex(map_.Rows() - 1, c + 1);
        ab = { a.x, baseY, a.z }; bb = { b.x, baseY, b.z };
        DrawTriangle3D(a, ab, b, side); DrawTriangle3D(b, ab, bb, side);
    }
    for (int r = 0; r < map_.Rows() - 1; ++r) {
        Vector3 a = vertex(r, 0); Vector3 b = vertex(r + 1, 0);
        Vector3 ab{ a.x, baseY, a.z }; Vector3 bb{ b.x, baseY, b.z };
        DrawTriangle3D(a, ab, b, side); DrawTriangle3D(b, ab, bb, side);
        a = vertex(r, map_.Cols() - 1); b = vertex(r + 1, map_.Cols() - 1);
        ab = { a.x, baseY, a.z }; bb = { b.x, baseY, b.z };
        DrawTriangle3D(a, b, ab, side); DrawTriangle3D(b, bb, ab, side);
    }

    // Very sparse contour hints. Avoid drawing every grid line; dense lines made
    // the old soft mode look like a broken tiled board instead of a heightfield.
    Color contour = themeMode_ == ThemeMode::Dark ? Color{ 255, 241, 255, 70 } : Color{ 92, 65, 91, 54 };
    const int step = std::max(5, std::max(map_.Rows(), map_.Cols()) / 9);
    for (int r = step; r < map_.Rows(); r += step) {
        for (int c = 1; c < map_.Cols(); ++c) DrawLine3D(vertex(r, c - 1, 0.038f), vertex(r, c, 0.038f), contour);
    }
    for (int c = step; c < map_.Cols(); c += step) {
        for (int r = 1; r < map_.Rows(); ++r) DrawLine3D(vertex(r - 1, c, 0.038f), vertex(r, c, 0.038f), contour);
    }

    // Clear semantic overlays. Unlike the old soft mode, obstacles and roads
    // are not merely color tints buried inside the surface.
    for (int idx = 0; idx < map_.Count(); ++idx) {
        const Cell& cell = map_.GetCell(idx);
        GridCoord g = map_.Coord(idx);
        Vector3 p = vertex(g.row, g.col, 0.075f);

        if (map_.IsWater(idx)) {
            Color water = themeMode_ == ThemeMode::Dark ? Color{ 82, 168, 218, 205 } : Color{ 88, 169, 218, 190 };
            DrawCubeV({ p.x, p.y + 0.01f, p.z }, { 0.78f, 0.035f, 0.78f }, water);
            continue;
        }

        if (map_.IsObstacle(idx)) {
            Color cap = themeMode_ == ThemeMode::Dark ? Color{ 42, 37, 58, 245 } : Color{ 117, 104, 120, 225 };
            DrawCubeV({ p.x, p.y + 0.16f, p.z }, { 0.62f, 0.32f, 0.62f }, cap);
            continue;
        }

        if (cell.terrain == TerrainType::Road) {
            Color road = themeMode_ == ThemeMode::Dark ? Color{ 255, 185, 127, 220 } : Color{ 226, 138, 102, 205 };
            DrawCubeV({ p.x, p.y + 0.012f, p.z }, { 0.74f, 0.040f, 0.74f }, road);
        } else if (cell.terrain == TerrainType::Forest && ((idx * 31 + g.row * 17 + g.col * 13) % 9 == 0)) {
            Color forest = themeMode_ == ThemeMode::Dark ? Color{ 94, 220, 158, 230 } : Color{ 55, 150, 108, 220 };
            DrawCylinder({ p.x, p.y + 0.06f, p.z }, 0.06f, 0.16f, 0.34f, 8, forest);
        } else if (cell.terrain == TerrainType::Mountain && ((idx * 23 + g.row * 11 + g.col * 19) % 11 == 0)) {
            Color peak = themeMode_ == ThemeMode::Dark ? Color{ 238, 223, 255, 235 } : Color{ 122, 96, 151, 210 };
            DrawCylinder({ p.x, p.y + 0.10f, p.z }, 0.14f, 0.02f, 0.42f, 4, peak);
        }
    }

    // Algorithm state is drawn as an explicit top layer.
    if (frame != nullptr) {
        Color closed = themeMode_ == ThemeMode::Dark ? Color{ 128, 114, 160, 165 } : Color{ 132, 108, 164, 150 };
        Color open = themeMode_ == ThemeMode::Dark ? Color{ 95, 219, 235, 185 } : Color{ 37, 156, 184, 165 };
        Color current = themeMode_ == ThemeMode::Dark ? Color{ 255, 137, 111, 240 } : Color{ 230, 79, 58, 235 };
        auto plate = [&](int cell, Color color) {
            GridCoord g = map_.Coord(cell);
            Vector3 p = vertex(g.row, g.col, 0.20f);
            DrawCubeV({ p.x, p.y, p.z }, { 0.66f, 0.070f, 0.66f }, color);
        };
...
void App::DrawPath3D(const std::vector<int>& path) const {
    ...
    auto pointFor = [&](int cell) {
        return CellWorldPosition(cell, 0.66f);
    };

    std::vector<Vector3> points;
    points.reserve(path.size());
    for (int cell : path) points.push_back(pointFor(cell));
    ...
    DrawPathTube3D(points, 0.065f, outline, core);
}
...
int App::PickCell3D(Vector2 mouse) const {
    ...
    if (smoothTerrain3D_) {
        // Soft terrain is a continuous visual heightfield, not a pile of cell
        // boxes. Pick by projecting the ray to the map's XZ plane, then convert
        // that XZ point to a grid cell. This avoids the old mismatch where the
        // ray tested raw cell boxes while the surface was drawn from smoothed
        // heights.
        if (std::fabs(ray.direction.y) < 1.0e-5f) return -1;
        ...
        const int col = static_cast<int>(std::floor(x + static_cast<float>(map_.Cols()) * 0.5f));
        const int row = static_cast<int>(std::floor(z + static_cast<float>(map_.Rows()) * 0.5f));
        if (!map_.InBounds(row, col)) return -1;
        return map_.Index(row, col);
    }
...
float App::SoftSurfaceHeightAt(int row, int col) const {
    ...
    for (int dr = -2; dr <= 2; ++dr) {
        for (int dc = -2; dc <= 2; ++dc) {
            ...
            float h = static_cast<float>(map_.GetCell(idx).height) / 10.0f;
            if (map_.IsWater(idx)) h *= 0.16f;
            const float dist = static_cast<float>(std::abs(dr) + std::abs(dc));
    ...
    const float scale = std::max(0.42f, terrainHeightScale_) * 2.80f;
    return 0.16f + std::pow(h01, 0.86f) * scale;
}
