...
void App::Run() {
    ...
    InitWindow(kWindowWidth, kWindowHeight, "PathLab Prototype v0.8.6 - rounded safe path curves");
...
void App::DrawTopBar() {
    ...
    DrawTextUI("v0.8.6", S(128), S(24), 15, MutedTextColor());
...
void App::DrawTerrain3D() {
    ...
    DrawRectangleLinesEx(canvas_, 1.0f, GridLineColor());
    DrawTextUI(smoothTerrain3D_ ? "3D contour-relief terrain" : "3D solid terrain editor", canvas_.x + S(22.0f), canvas_.y + S(22.0f), 19, TextColor());
    DrawTextUI("Soft mode now uses banded relief, stronger shading, and clearer overlays.", canvas_.x + S(22.0f), canvas_.y + S(49.0f), 14, MutedTextColor());
...
void App::DrawSmoothTerrain3D(const SearchFrame* frame) const {
    // v0.8.6: Soft mode is no longer a raw pastel mesh. It is rendered as a
    // contour-relief terrain: one calm base material, stronger lighting,
    // readable height bands, and separate overlays for roads/water/obstacles.
    // The path-planning graph is still the same grid; only the visual layer changes.

    auto rawHeight01 = [&](int idx) {
        if (idx < 0 || idx >= map_.Count()) return 0.0f;
        float h = static_cast<float>(map_.GetCell(idx).height) / 10.0f;
        if (map_.IsWater(idx)) h *= 0.18f;
        return std::clamp(h, 0.0f, 1.0f);
    };

    auto smoothHeight01 = [&](int row, int col) {
        // A weighted 5x5 kernel removes the old jagged triangle noise while
        // preserving large-scale hills and valleys.
        float sum = 0.0f;
        ...
        for (int dr = -2; dr <= 2; ++dr) {
            for (int dc = -2; dc <= 2; ++dc) {
                ...
                sum += rawHeight01(map_.Index(rr, cc)) * w;
                weightSum += w;
            }
        }
    ...
    const float visualHeightScale = std::max(0.42f, terrainHeightScale_) * 3.45f;

    auto displayHeight = [&](int row, int col) {
        const float h = smoothHeight01(row, col);
        // A slight curve gives low hills visible relief without turning high
        // mountains into spikes.
        return 0.10f + std::pow(h, 0.92f) * visualHeightScale;
    };
    ...
    // Height bands / contour lines. These are sparse but high-contrast, so the
    // user can read the relief even on a projector.
    Color contour = themeMode_ == ThemeMode::Dark ? Color{ 255, 241, 255, 120 } : Color{ 92, 65, 91, 112 };
    const int step = std::max(2, std::max(map_.Rows(), map_.Cols()) / 22);
    for (int r = 0; r < map_.Rows(); r += step) {
        for (int c = 1; c < map_.Cols(); ++c) DrawLine3D(vertex(r, c - 1, 0.032f), vertex(r, c, 0.032f), contour);
    }
    for (int c = 0; c < map_.Cols(); c += step) {
        for (int r = 1; r < map_.Rows(); ++r) DrawLine3D(vertex(r - 1, c, 0.032f), vertex(r, c, 0.032f), contour);
    }
