...
void App::Run() {
    ...
    InitWindow(kWindowWidth, kWindowHeight, "PathLab Prototype v0.8 - dream-gradient terrain themes");
...
void App::DrawTopBar() {
    ...
    DrawTextUI("PathLab Prototype v0.8", S(20), S(15), 26, TextColor());
    DrawTextUI("dream-gradient terrain + light/dark theme", S(255), S(22), 17, MutedTextColor());
...
void App::DrawSmoothTerrain3D(const SearchFrame* frame) const {
    for (int r = 0; r < map_.Rows() - 1; ++r) {
        for (int c = 0; c < map_.Cols() - 1; ++c) {
            ...
            if (blocked) {
                ...
                DrawCubeV(center, { 0.72f, 0.20f, 0.72f }, themeMode_ == ThemeMode::Dark ? Color{ 70, 63, 92, 205 } : Color{ 160, 140, 153, 190 });
            }
        }
    }
    ...
    for (int r = 0; r < map_.Rows(); r += stride) {
        for (int c = 1; c < map_.Cols(); c += stride) {
            DrawLine3D(TerrainVertexWorld(r, c - 1, 0.018f), TerrainVertexWorld(r, c, 0.018f), (themeMode_ == ThemeMode::Dark ? Color{ 255, 240, 220, 32 } : Color{ 100, 80, 105, 34 }));
        }
    }
    for (int c = 0; c < map_.Cols(); c += stride) {
        for (int r = 1; r < map_.Rows(); r += stride) {
            DrawLine3D(TerrainVertexWorld(r - 1, c, 0.018f), TerrainVertexWorld(r, c, 0.018f), (themeMode_ == ThemeMode::Dark ? Color{ 255, 240, 220, 32 } : Color{ 100, 80, 105, 34 }));
        }
    }
}
...
void App::DrawTerrainFeature3D(int cell) const {
    ...
    switch (c.terrain) {
        case TerrainType::Road: {
            ...
            DrawCubeV({ p.x, p.y + 0.012f, p.z }, { 0.72f, 0.035f, 0.72f }, slab);
            DrawCubeV({ p.x, p.y + 0.034f, p.z }, { 0.08f, 0.018f, 0.54f }, stripe);
            break;
        }
        case TerrainType::Forest: {
            ...
            DrawCylinder({ p.x, p.y + 0.10f, p.z }, 0.055f, 0.055f, 0.22f, 7, trunk);
            DrawCylinder({ p.x, p.y + 0.36f, p.z }, 0.0f, 0.24f, 0.46f, 8, canopy);
            break;
        }
        case TerrainType::Mountain: {
            ...
            DrawCylinder({ p.x, p.y + 0.24f, p.z }, 0.0f, 0.33f, 0.54f, 6, peak);
            DrawSphere({ p.x + 0.10f, p.y + 0.15f, p.z - 0.10f }, 0.105f, stone);
            break;
        }
        case TerrainType::Water: {
