...
void DrawShadedCubeV(Vector3 center, Vector3 size, Color base) {
    ...
    // Solid ambient face lighting. The previous high-contrast version made the lower/
    // rear faces almost black in the pastel theme, which looked hollow rather than solid.
    // Keep three visible planes distinct, but avoid a harsh black underside.
    const Color top    = ScaleColor(base, 1.08f);
    const Color east   = ScaleColor(base, 0.92f);
    const Color west   = ScaleColor(base, 0.78f);
    const Color south  = ScaleColor(base, 0.98f);
    const Color north  = ScaleColor(base, 0.84f);
    const Color bottom = ScaleColor(base, 0.76f);
...
void App::Run() {
    ...
    InitWindow(kWindowWidth, kWindowHeight, "PathLab Prototype v1.0.2 - directional face lighting");
...
void App::DrawTopBar() {
    ...
    DrawTextUI("v1.0.2", S(128), S(24), 15, MutedTextColor());
