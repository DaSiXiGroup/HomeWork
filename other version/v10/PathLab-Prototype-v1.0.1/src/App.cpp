...
void DrawShadedCubeV(Vector3 center, Vector3 size, Color base) {
    ...
    // Hand-authored face lighting. This is more readable than raylib's single-color
    // DrawCubeV for our low-poly diorama style: the top is bright, one side is mid-tone,
    // and the other side is deliberately darker, so every cuboid has real 3D volume.
    const Color top    = ScaleColor(base, 1.16f);
    const Color east   = ScaleColor(base, 0.86f);
    const Color west   = ScaleColor(base, 0.66f);
    const Color south  = ScaleColor(base, 0.96f);
    const Color north  = ScaleColor(base, 0.74f);
    const Color bottom = ScaleColor(base, 0.52f);

    DrawQuad3D(v010, v110, v111, v011, top);       // top
    DrawQuad3D(v000, v001, v101, v100, bottom);    // bottom
...
void DrawFlatShadedCubeV(Vector3 center, Vector3 size, Color base) {
    // For very thin slabs, keep the lighting gentler so roads/water remain clean,
    // but still avoid the cardboard-flat look.
    const Color top = ScaleColor(base, 1.10f);
    const Color side = ScaleColor(base, 0.78f);
...
void App::Run() {
    ...
    InitWindow(kWindowWidth, kWindowHeight, "PathLab Prototype v1.0.1 - directional face lighting");
...
void App::DrawTopBar() {
    ...
    DrawTextUI("v1.0.1", S(128), S(24), 15, MutedTextColor());
