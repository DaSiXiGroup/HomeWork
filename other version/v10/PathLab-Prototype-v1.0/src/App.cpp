...
void App::Run() {
    ...
    InitWindow(kWindowWidth, kWindowHeight, "PathLab Prototype v1.0 - editor and diorama modes");
...
void App::DrawTopBar() {
    ...
    DrawTextUI("v1.0", S(128), S(24), 15, MutedTextColor());
...
void App::DrawSmoothTerrain3D(const SearchFrame* frame) const {
    ...
    DrawCubeV({ 0.0f, -0.18f, 0.0f }, { worldW + 1.2f, 0.30f, worldH + 1.2f }, baseSide);
    DrawCubeV({ 0.0f, 0.015f, 0.0f }, { worldW + 0.95f, 0.035f, worldH + 0.95f }, baseTop);
    ...
    for (int idx = 0; idx < map_.Count(); ++idx) {
        ...
        if (cell.terrain == TerrainType::Water) {
            ...
            DrawCubeV({ p.x, 0.075f, p.z }, { 0.96f, 0.030f, 0.96f }, water);
            continue;
        }
        ...
        if (cell.terrain == TerrainType::Road) {
            ...
            DrawCubeV({ p.x, 0.125f, p.z }, { 0.94f, 0.045f, 0.94f }, road);
            ...
            if (horizontal && !vertical) DrawCubeV({ p.x, 0.154f, p.z }, { 0.52f, 0.012f, 0.055f }, stripe);
            else if (vertical && !horizontal) DrawCubeV({ p.x, 0.154f, p.z }, { 0.055f, 0.012f, 0.52f }, stripe);
            else DrawCubeV({ p.x, 0.154f, p.z }, { 0.20f, 0.012f, 0.20f }, stripe);
            continue;
        }
    ...
    for (int idx = 0; idx < map_.Count(); ++idx) {
        ...
        DrawCubeV({ p.x, h * 0.5f + 0.08f, p.z }, { 0.70f, h, 0.70f }, building);
        DrawCubeV({ p.x, h + 0.097f, p.z }, { 0.74f, 0.035f, 0.74f }, roof);
    }
    ...
    if (frame != nullptr) {
        ...
        for (int idx = 0; idx < map_.Count(); idx += stride) {
            ...
            DrawCubeV({ p.x, p.y, p.z }, { 0.82f, 0.026f, 0.82f }, ov);
        }
    }
}
