...
void App::Run() {
    ...
    InitWindow(kWindowWidth, kWindowHeight, "PathLab Prototype v0.5 - 2D/3D path planning visualization");
...
void App::ToggleViewMode() {
    ...
    if (view3D_) {
        ResetCamera3D();
        statusMessage_ = "3D terrain view enabled. Right/middle drag rotates, wheel zooms, WASD pans.";
    } else {
        statusMessage_ = "2D map view enabled. Editing tools are available in 2D mode.";
    }
}
...
void App::DrawBottomPanel() {
    ...
    DrawTextUI(view3D_ ? "3D: right/middle drag rotate | wheel zoom | WASD pan | Q/E height | R reset | F5 2D" : "2D: Left paint | Right erase | Shift+Left start | Ctrl+Left goal | F5 3D", 
...
void App::DrawTerrain3D() {
    ...
    for (int r = 0; r < map_.Rows(); ++r) {
        for (int c = 0; c < map_.Cols(); ++c) {
            ...
            float h = 0.12f + static_cast<float>(cell.height) * terrainHeightScale_;
            if (blocked) h = std::max(0.65f, h + 0.55f);
            if (cell.terrain == TerrainType::Water) h = std::max(0.18f, h * 0.32f);
    ...
    DrawTextUI("3D terrain view", canvas_.x + S(22.0f), canvas_.y + S(22.0f), 20, TextColor());
    DrawTextUI("This is a rendering view. Use F5 to return to 2D editing.", canvas_.x + S(22.0f), canvas_.y + S(50.0f), 15, MutedTextColor());
}
