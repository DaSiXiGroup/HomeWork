...
private:
    ...
    std::vector<AgentComparisonRow> agentComparisonRows_;
    std::vector<MapHistoryState> undoStack_;
    std::vector<MapHistoryState> redoStack_;
    ...
    bool showAgentComparison_ = false;
    int lastEditedCell_ = -1;
    ...
    void DrawPath3DColored(const std::vector<int>& path, Color core, float radius = 0.055f) const;
    void DrawAgentComparison2D(float startX, float startY, float cellSize) const;
    void DrawAgentComparison3D() const;
    void DrawLegend(float x, float& y, float contentW);
    ...
    void GenerateCityPreset();
    void GenerateRiverPreset();
    void GenerateMountainPreset();
    void RunDemoMode();
    void ApplyMapSize();
    ...
    void PushUndoState(const std::string& label);
    void ClearHistory();
    void UndoEdit();
    void RedoEdit();
    ...
    [[nodiscard]] float SoftSurfaceHeightAt(int row, int col) const;
    [[nodiscard]] float CellVisualHeight3D(int cell) const;
