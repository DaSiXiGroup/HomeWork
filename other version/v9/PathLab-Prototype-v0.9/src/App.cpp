...
void App::Run() {
    ...
    InitWindow(kWindowWidth, kWindowHeight, "PathLab Prototype v0.9 - terrace relief terrain");
...
void App::ResetCamera3D() {
    ...
    // Solid view is an editor-like overview. Relief view is a terrain showcase,
    // so it uses a closer, lower camera to reveal height relief instead of
    // collapsing the surface into a flat pastel plane.
    cameraTarget_ = { 0.0f, smoothTerrain3D_ ? 0.35f : 0.0f, 0.0f };
    cameraDistance_ = std::max(smoothTerrain3D_ ? 26.0f : 28.0f, mapSize * (smoothTerrain3D_ ? 0.88f : 0.95f));
    cameraYaw_ = smoothTerrain3D_ ? 0.78f : 0.78f;
    cameraPitch_ = smoothTerrain3D_ ? 0.60f : 0.58f;

    camera_.up = { 0.0f, 1.0f, 0.0f };
    camera_.fovy = smoothTerrain3D_ ? 42.0f : 45.0f;
...
void App::DrawTopBar() {
    ...
    DrawTextUI("v0.9", S(128), S(24), 15, MutedTextColor());
...
void App::DrawLeftPanel() {
    ...
    y += S(40);
    if (ToggleButton({ x, y, bw, S(34) }, smoothTerrain3D_ ? "Surface: relief" : "Surface: solid", smoothTerrain3D_)) {
        smoothTerrain3D_ = !smoothTerrain3D_;
        if (view3D_) ResetCamera3D();
        statusMessage_ = smoothTerrain3D_ ? "Relief terrain mode: clean terraced height board" : "Solid terrain mode enabled";
    }
...
void App::DrawTerrain3D() {
    ...
    DrawRectangleLinesEx(canvas_, 1.0f, GridLineColor());
    DrawTextUI(smoothTerrain3D_ ? "3D terrace relief terrain" : "3D solid terrain editor", canvas_.x + S(22.0f), canvas_.y + S(22.0f), 19, TextColor());
    DrawTextUI(smoothTerrain3D_ ? "Relief mode uses readable terraced blocks instead of the failed soft mesh." : "Solid mode uses cell blocks for precise editing.", canvas_.x + S(22.0f), canvas_.y + S(49.0f), 14, MutedTextColor());
...
void App::DrawSmoothTerrain3D(const SearchFrame* frame) const {
    // v0.9: the old "soft mesh" design is removed. This mode is now a clean
    // terrace-relief board: no triangle patchwork, no invisible heightfield,
    // no duplicate feature clutter. It intentionally stays discrete enough to
    // be readable while giving a stronger 2.5D terrain impression than solid
    // editor blocks.

    auto cellTopColor = [&](int idx) {
        const Cell& cell = map_.GetCell(idx);
        const float h = std::clamp(static_cast<float>(cell.height) / 9.0f, 0.0f, 1.0f);

        Color low = themeMode_ == ThemeMode::Dark ? Color{ 47, 49, 76, 255 } : Color{ 252, 226, 208, 255 };
        Color mid = themeMode_ == ThemeMode::Dark ? Color{ 94, 82, 127, 255 } : Color{ 230, 192, 155, 255 };
        Color high = themeMode_ == ThemeMode::Dark ? Color{ 205, 185, 242, 255 } : Color{ 168, 133, 188, 255 };
        Color base = h < 0.55f ? Blend(low, mid, h / 0.55f) : Blend(mid, high, (h - 0.55f) / 0.45f);

        if (cell.terrain == TerrainType::Road) {
            return themeMode_ == ThemeMode::Dark ? Color{ 230, 154, 104, 255 } : Color{ 226, 128, 91, 255 };
        }
        if (cell.terrain == TerrainType::Forest) {
            return themeMode_ == ThemeMode::Dark ? Color{ 83, 175, 135, 255 } : Color{ 105, 175, 132, 255 };
        }
        if (cell.terrain == TerrainType::Mountain) {
            return themeMode_ == ThemeMode::Dark ? Color{ 201, 187, 230, 255 } : Color{ 160, 133, 184, 255 };
        }
        if (cell.terrain == TerrainType::Water) {
            return themeMode_ == ThemeMode::Dark ? Color{ 76, 143, 191, 255 } : Color{ 96, 171, 213, 255 };
        }
        return base;
    };

    auto applyFrameColor = [&](int idx, Color color) {
        if (frame == nullptr) return color;
        const Color closed = themeMode_ == ThemeMode::Dark ? Color{ 122, 111, 158, 210 } : Color{ 156, 136, 186, 190 };
        const Color open = themeMode_ == ThemeMode::Dark ? Color{ 92, 208, 224, 220 } : Color{ 67, 178, 199, 200 };
        const Color path = themeMode_ == ThemeMode::Dark ? Color{ 255, 205, 132, 255 } : Color{ 238, 91, 64, 255 };
        const Color current = themeMode_ == ThemeMode::Dark ? Color{ 255, 136, 112, 255 } : Color{ 231, 82, 62, 255 };
        if (ContainsCell(frame->closed, idx)) color = Blend(color, closed, 0.50f);
        if (ContainsCell(frame->open, idx)) color = Blend(color, open, 0.55f);
        if (ContainsCell(frame->path, idx)) color = path;
        if (frame->current == idx) color = current;
        return color;
    };

    // Draw a continuous-looking base slab first. This makes the terrain read as
    // one object rather than a pile of floating tiles.
    const float worldW = static_cast<float>(map_.Cols());
    const float worldH = static_cast<float>(map_.Rows());
    const Color base = themeMode_ == ThemeMode::Dark ? Color{ 30, 27, 46, 255 } : Color{ 238, 200, 184, 255 };
    DrawCubeV({ 0.0f, -0.12f, 0.0f }, { worldW, 0.18f, worldH }, base);

    // Main terraced height board. Use nearly full-size cells so there are no
    // distracting holes; use no per-cell wire grid by default.
    for (int r = 0; r < map_.Rows(); ++r) {
        for (int c = 0; c < map_.Cols(); ++c) {
            const int idx = map_.Index(r, c);
            const Cell& cell = map_.GetCell(idx);
            ...
            float h = CellVisualHeight3D(idx);
            if (cell.terrain == TerrainType::Water) h = std::min(h, 0.22f);
            if (map_.IsObstacle(idx) && cell.terrain != TerrainType::Water) h = std::max(h, 0.58f);
            ...
            const Vector3 center{ x, h * 0.5f, z };

            Color color = applyFrameColor(idx, cellTopColor(idx));
            if (map_.IsObstacle(idx) && cell.terrain != TerrainType::Water) {
                color = themeMode_ == ThemeMode::Dark ? Color{ 60, 55, 78, 255 } : Color{ 148, 132, 148, 255 };
            }
            DrawCubeV(center, Vector3{ 0.985f, h, 0.985f }, color);

            // Only important semantic boundaries get outlines. Avoid the full
            // dense grid because it made the previous soft mode unreadable.
            if (blocked || idx == map_.Start() || idx == map_.Goal()) {
                Color line = themeMode_ == ThemeMode::Dark ? Color{ 255, 238, 214, 85 } : Color{ 79, 59, 84, 100 };
                DrawCubeWiresV(center, Vector3{ 0.99f, h + 0.006f, 0.99f }, line);
            }
        }
    }

    // Roads and water are drawn as readable top-layer plates, not buried in a
    // smooth color tint. This keeps the map semantic even at a distance.
    for (int idx = 0; idx < map_.Count(); ++idx) {
        const Cell& cell = map_.GetCell(idx);
        if (cell.terrain != TerrainType::Road && cell.terrain != TerrainType::Water) continue;
        Vector3 p = CellWorldPosition(idx, 0.055f);
        if (cell.terrain == TerrainType::Road) {
            Color road = themeMode_ == ThemeMode::Dark ? Color{ 255, 184, 118, 230 } : Color{ 255, 170, 118, 215 };
            DrawCubeV({ p.x, p.y, p.z }, Vector3{ 0.72f, 0.035f, 0.72f }, road);
        } else {
            Color water = themeMode_ == ThemeMode::Dark ? Color{ 93, 178, 226, 220 } : Color{ 91, 181, 226, 205 };
            DrawCubeV({ p.x, p.y, p.z }, Vector3{ 0.86f, 0.030f, 0.86f }, water);
        }
    }

    // Sparse natural markers. The board itself carries the terrain meaning;
    // markers are only decorative accents, so keep them very sparse.
    for (int idx = 0; idx < map_.Count(); ++idx) {
        const Cell& cell = map_.GetCell(idx);
        GridCoord g = map_.Coord(idx);
        if (cell.terrain == TerrainType::Forest && ((idx * 31 + g.row * 17 + g.col * 13) % 23 == 0)) {
            Vector3 p = CellWorldPosition(idx, 0.12f);
            DrawCylinder({ p.x, p.y, p.z }, 0.045f, 0.13f, 0.27f, 7,
                themeMode_ == ThemeMode::Dark ? Color{ 103, 221, 161, 230 } : Color{ 62, 151, 108, 220 });
        } else if (cell.terrain == TerrainType::Mountain && ((idx * 23 + g.row * 11 + g.col * 19) % 29 == 0)) {
            Vector3 p = CellWorldPosition(idx, 0.14f);
            DrawCylinder({ p.x, p.y, p.z }, 0.11f, 0.02f, 0.32f, 4,
                themeMode_ == ThemeMode::Dark ? Color{ 235, 223, 255, 225 } : Color{ 122, 96, 151, 205 });
        }
    }
}
...
float App::SoftSurfaceHeightAt(int row, int col) const {
    ...
    // Relief mode intentionally uses a lightly smoothed, quantized height. The
    // quantization creates clean terraces instead of noisy, unreadable triangles.
    float sum = 0.0f;
    ...
    for (int dr = -1; dr <= 1; ++dr) {
        for (int dc = -1; dc <= 1; ++dc) {
            ...
            const float w = (dr == 0 && dc == 0) ? 4.0f : ((std::abs(dr) + std::abs(dc)) == 1 ? 1.4f : 0.7f);
            sum += std::clamp(h, 0.0f, 1.0f) * w;
            weightSum += w;
        }
    }

    float h01 = weightSum > 0.0f ? sum / weightSum : 0.0f;
    h01 = std::round(h01 * 6.0f) / 6.0f;
    const float scale = std::max(0.18f, terrainHeightScale_) * 2.15f;
    return 0.12f + h01 * scale;
}
